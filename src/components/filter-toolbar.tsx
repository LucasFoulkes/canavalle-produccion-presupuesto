      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-56">
          ...
          <DropdownMenuRadioGroup value={column} onValueChange={setColumn as any}>
            <DropdownMenuRadioItem value="*">Todas las columnas</DropdownMenuRadioItem>
            {columns.map((col) => (
        ...
        <div className="flex items-center gap-1 whitespace-nowrap overflow-x-auto scrollbar-thin pr-2">
          {filters.map((filter) => {
            ...
            const valueLabel = filter.type === 'date'
              ? (filter.op === 'between'
                ? `${renderValue(filter.value)} – ${renderValue(filter.value2 ?? '')}`
...
const CaretDownIcon = ({ className, ...props }: React.ComponentProps<'svg'>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 16 16"
    fill="none"
